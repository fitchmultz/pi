...
All awaited extension handlers and core `turn_end` subscribers finish before a turn hold. Settlement handlers finish before a settled hold, including when they temporarily expose `isIdle`. Native notification handlers, extension event-bus callbacks and `pi.exec` operations also own their pending activity. Settings writes are flushed and recorded errors reject acquisition. Unresolved dirty fields or settings load failures remain blocking even after presentation errors are drained; only successful persistence or explicit reload/trust reconciliation clears that state. Native catalog/auth operations and their underlying file locks are joined, including cancellation cleanup; new operations invalidate the hold before starting. Catalog refresh failures may retain the previous native cached catalog, exactly as normal Pi startup does. There is no separate host model store.

Native journal write failures retain accepted entries in memory and still report the I/O error. Before live or clean-exit capture, Pi retries persistence from those entries; an ongoing failure rejects capture and releases the hold. Repair the filesystem and retry capture, not the failed append. Successful retry preserves entry IDs, history, labels and the selected leaf without replaying events. Journals intentionally deferred until the first assistant response remain deferred and are included in the artifact.
...
The path must be absolute with an existing parent directory. At CLI startup Pi loads any `--checkpoint` input **before** atomically removing its previous `completedExit` marker, including when both paths are identical. The native artifact data is retained so a failed restore can be retried; an unmarked file is **not** completed-exit proof. An existing output must be a valid native checkpoint, not an unrelated file or journal. Old proof is also revoked when another restore input fails. The exit writer refuses to replace the native journal.
...
Late native UI/model/catalog/extension activity during final flush invalidates the candidate before publication. A ref'ed 30-second cleanup budget prevents an unfinished memory-only callback from producing implicit successful process exit. Shutdown extension errors, native persistence errors, unfinished callbacks, unsupported input, runtime-only selected-provider keys, or artifact-write errors print `Clean-exit checkpoint failed: ...` and exit nonzero without a new completed artifact. OAuth/provider callback failures before a credential write do not poison final capture: native auth never adopted that failed refresh. Actual storage failures still fail closed. The default exit behavior is unchanged when the variable is unset.
