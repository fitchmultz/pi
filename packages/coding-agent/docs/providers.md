...
On Linux, credential saves require GNU `cp` (from `coreutils`) and `/usr/bin/setfacl` (from `acl`) to preserve access controls and security labels. The directory containing `auth.json` must be writable; a single-file bind mount cannot be replaced atomically.
