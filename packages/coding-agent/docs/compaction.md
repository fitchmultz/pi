...
The provider error code `misalignment_policy_violation` stops the affected session. Pi preserves the failed response and its request/response IDs, retains queued input, and stops automatic retries, compaction, cache warming, and fresh-context continuation. A stop received during compaction, branch summarization, or cache warming is also recorded. Context edits and tree navigation do not clear the stop; read-only inspection remains available. History forks inherit the stop, including forks from an earlier entry or before the first message. Related new sessions that name the stopped session as their parent also inherit it. Unrelated fresh empty sessions are unaffected. Review the recorded actions and error information; stopping does not undo actions already completed.
