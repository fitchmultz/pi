...
Fired after user submits prompt, before the pre-prompt context budget check and agent loop. Can inject a message and/or modify the system prompt. Also fires for an idle `pi.sendMessage(..., { triggerTurn: true })` wakeup, with `event.prompt === ""`.
...
The `systemPromptOptions` field gives extensions access to the same structured data Pi uses to build the system prompt. Collections are mutable. Prefer changing `sections`, `selectedTools`, or `promptGuidelines`: Pi diffs the resulting prompt sections against what the model already has and appends one system message patching only the changed sections. Returning `systemPrompt`, or setting `forceSystemPrompt`, replaces the whole prompt for the run: every provider receives the forced text as its leading system prompt (a cache miss when it changes), and the session transcript keeps recording the structured sections. The forced text survives tool-loop turns and fresh context windows within that run, then ends when the run settles; it is regenerated by handlers on the next run, not restored from new session records. Because forcing replaces the complete prompt, later section edits do not amend it: use sections for additive guidance and reserve forcing for intentional full replacement. Tool selection changes update both the prompt contributions and executable provider tools; calling `pi.setActiveTools()` inside the handler has the same effect as editing `selectedTools`. Models that accept system messages mid-conversation receive the patch in place and keep their cached prefix; other models get the replayed prompt as their system prompt, which is a cache miss once per change.
...
pi.on("agent_settled", async (_event, ctx) => {
  // ctx.isIdle() is true unless shutdown began; requested runs wait for all settled handlers.
});
```
...
Returning `undefined` continues to the next handler, then local execution if none handles the event. A valid result stops propagation: `operations` executes the command through the supplied backend, while `result` records the completed command without executing it. Return exactly one of them. A thrown error or invalid result stops the command without falling back to local execution, in interactive, RPC, and SDK use.
...
`ctx.scopedModels` is the read-only list of models scoped to the current session — the same set the `/scoped-models` command shows. It is resolved at session start from the `--models` CLI flag and the `enabledModels` setting (matched with minimatch on `provider/modelId` or a bare `modelId`). Scope matching retains models with failed auth checks so startup does not silently change providers; those models are not advertised as configured by `getAvailable()`. It is empty when no scoping is configured, meaning every available model is usable. Each entry is `{ model, thinkingLevel? }`, where `thinkingLevel` is set only when a pattern pinned it (e.g. `anthropic/*:high`). Use it to populate a model picker that mirrors the built-in one instead of enumerating the whole catalogue via `ctx.modelRegistry.getAvailable()`.
