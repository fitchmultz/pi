...
Pi validates each hook's entire draft batch before appending its edits, then refreshes canonical context before provider dispatch. Promises, invalid drafts, and thrown errors stop dispatch; the window marker and earlier hooks' accepted edits remain committed. Canonical state and `context_window_started` reflect that cut even when a hook fails. Hooks cannot request another window or continuation. This synchronous boundary also sees receipts completed during an awaited automatic-compaction handler.
