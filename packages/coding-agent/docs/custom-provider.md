...
A throwing `check` is distinct from an unconfigured (`undefined`) result. All-provider availability omits the failed provider, retains healthy providers, and records the diagnostic without inventing auth type/source or reusing old subscription metadata. Direct provider checks and resolution still reject. Existing saved/default models and explicitly scoped choices survive a failed check if the exact model still exists; normal prompt admission reports the provider's error instead of selecting another payer. Successful unconfigured checks retain ordinary fallback behavior. Cancellation and credential-store failures are not converted to partial provider success.
