...
Full rewrites stage the complete journal in an exclusively created sibling temporary file and replace the journal only after all writes and close succeed. Failed repair writes leave the prior journal bytes intact and remove the temporary file. Repair still requires a writable journal and preserves its permission mode; initial creation retains exclusive collision protection.
