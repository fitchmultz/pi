...
describe("session context edits", () => {
	...
	it("projects edits within native windows without replaying a retained window's older loadout", () => {
		const session = SessionManager.inMemory();
		...
		const windowId = session.appendContextWindow("native handoff", 100);
		const rawWindow = session.getEntry(windowId);
		...
		session.appendCompaction("current summary", windowId, 80);
		session.appendContextEdit(inputId, { content: "edited current input" });
		const projection = session.buildSessionProjection();
		expect(projection.entries.find((entry) => entry.sourceEntry.id === windowId)?.sourceEntry).toBe(rawWindow);
		...
		expect(JSON.stringify(projection.messages)).toContain("native handoff");
		...
		expect(rawWindow).toMatchObject({ systemMessage: { toolsAdded: [{ name: "old" }] } });
		...
		session.appendContextWindow("second native handoff", 50);
